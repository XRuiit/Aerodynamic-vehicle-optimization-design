...
%%CST参数化
x = linspace(0, 1, 200);
a_upper = [0.12, 0.14, 0.19, 0.16,0.2,0.2]; % 6个系数
a_lower = [-0.12, -0.14, -0.21, -0.1,-0.15,0.1];
...
%% 计算误差
% 计算参数化翼型在RAE2822的x坐标处的坐标
[z_upper_rae, z_lower_rae] = cst_airfoil(x_upper, a_upper, a_lower);

% 计算误差
error_upper = y_upper - z_upper_rae(:)';  % 确保列向量转行向量
error_lower = y_lower - z_lower_rae(:)';

mae_upper = mean(abs(error_upper));     % 上表面平均绝对误差
mae_lower = mean(abs(error_lower));     % 下表面平均绝对误差
rmse_upper = sqrt(mean(error_upper.^2)); % 上表面均方根误差
rmse_lower = sqrt(mean(error_lower.^2)); % 下表面均方根误差

% 输出结果
fprintf('上表面平均绝对误差（MAE）: %.6f\n', mae_upper);
fprintf('下表面平均绝对误差（MAE）: %.6f\n', mae_lower);
